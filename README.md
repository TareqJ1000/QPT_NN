...
- datamash.py is a relatively simple script that combines two datasets together. This is used, in particular, to create augumented datasets for two-step training 
- synthetic_utils.py holds all the functions related to the fundamental generation of synthetic processes and is used prominitely in datagen as well as in the data analysis notebooks (see below)  

Use of the datagen.py, train.py, and datamash.py scripts requires configuration of, respectively, the 'datagen.yaml', 'train.yaml', and 'datamash_simple.yaml' configuration files, which can be found in the configs folder. The files can be configured to generate experiments and to train networks of arbitrary complexity and to the users liking.
